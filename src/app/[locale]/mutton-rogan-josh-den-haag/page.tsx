...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const descriptions = {
    en: 'Authentic mutton rogan josh in Den Haag at Chopras. Tender lamb in aromatic Kashmiri spice sauce. Slow-cooked daily. Halal. Order online or visit Leyweg.',
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/${locale}/mutton-rogan-josh-den-haag`,
      languages: { en: `${SITE_URL}/en/mutton-rogan-josh-den-haag`, nl: `${SITE_URL}/nl/mutton-rogan-josh-den-haag`, 'x-default': `${SITE_URL}/en/mutton-rogan-josh-den-haag` },
    },
  }
}
...
export default function MuttonRoganJoshPage({ params }: Props) {
  ...
  return (
    <>
      <JsonLd data={getLocalRestaurantSchema(locale, ['Den Haag'], `${SITE_URL}/${locale}/mutton-rogan-josh-den-haag`)} />
      <JsonLd data={getBreadcrumbSchema([
        { name: tr.common.nav.home, item: `${SITE_URL}/${locale}` },
        { name: isNl ? 'Rogan Josh Den Haag' : 'Rogan Josh Den Haag', item: `${SITE_URL}/${locale}/mutton-rogan-josh-den-haag` },
      ])} />
      ...
      <section className="bg-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link href={`${base}/dal-makhani-den-haag`} className="inline-block border-2 border-[#1B2B5E] text-[#1B2B5E] px-8 py-4 rounded-full font-bold hover:bg-[#1B2B5E] hover:text-white transition-colors text-center">
