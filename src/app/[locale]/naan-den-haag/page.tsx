...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const descriptions = {
    en: 'Naan at Chopras Den Haag. Garlic naan, tandoori naan, plain naan. Baked fresh in clay oven. Order now from Leyweg 986.',
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/${locale}/naan-den-haag`,
      languages: {
        en: `${SITE_URL}/en/naan-den-haag`,
        nl: `${SITE_URL}/nl/naan-den-haag`,
        'x-default': `${SITE_URL}/en/naan-den-haag`,
      },
    },
  }
}
...
export default function NaanPage({ params }: Props) {
  ...
  return (
    <>
      <JsonLd data={getLocalRestaurantSchema(locale, ['Den Haag'], `${SITE_URL}/${locale}/naan-den-haag`)} />
      <JsonLd data={getBreadcrumbSchema([
        { name: tr.common.nav.home, item: `${SITE_URL}/${locale}` },
        { name: isNl ? 'Naan Den Haag' : 'Naan Den Haag', item: `${SITE_URL}/${locale}/naan-den-haag` },
      ])} />
      ...
      <section className="bg-[#1B2B5E] py-20 text-center">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="inline-flex items-center gap-2 mb-6">
            ...
            <span className="text-[#D4AF37] text-xs uppercase tracking-widest font-medium" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.8), 0 0 8px rgba(0,0,0,0.5)' }}>INDIAN BREAD</span>
            <div className="h-px w-8 bg-[#D4AF37]" />
          </div>
          ...
          <div className="prose prose-lg max-w-none text-[#1A1A1A] space-y-5">
            {isNl ? (
              <>
                ...
                <p>Naan Den Haag kost ongeveer vier tot zes euro per stuk, afhankelijk van wat je erop hebt. Knoflook naan. Tandoori naan. Peshwari naan met noten en rozijn. Dit zijn alle variaties die mogelijk zijn omdat je de basis tandoor naan eerste moet hebben. En de basis tandoor naan moet perfect zijn.</p>
              </>
            ) : (
              <>
                ...
                <p>Naan Den Haag costs about four to six euros per piece, depending on what you put on it. Garlic naan. Tandoori naan. Peshwari naan with nuts and raisins. These are all variations that are possible because you must have the base tandoor naan first. And the base tandoor naan must be perfect.</p>
              </>
            )}
          </div>
        </div>
      ...
      <section className="bg-[#FFFAF5] py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link href={`${base}/butter-chicken-den-haag`} className="inline-block border-2 border-[#1B2B5E] text-[#1B2B5E] px-8 py-4 rounded-full font-bold hover:bg-[#1B2B5E] hover:text-white transition-colors text-center">
